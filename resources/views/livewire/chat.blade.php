...
<div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            {{-- chat container area --}}
            <div id="chat-wrapper" class="w-full bg-white rounded-lg shadow-lg p-6">
                <!-- Chat Messages -->
                <div id="chat-container" class="flex flex-col gap-4 pb-6 max-h-[500px] overflow-y-auto">
                    @foreach ($messages as $message)
                        @if ($message->sender->id !== auth()->user()->id)
                            <!-- Receiver Other User's Messages -->
                ...
                <form wire:submit="sendMessage">
                    <!-- Input Area -->
                    <div id="chat-input" class="flex items-center gap-3 p-3 bg-gray-50 rounded-full border border-gray-200">
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
                            <path d="M6.05 17.6C6.05 15.3218 8.26619 13.475 11 13.475C13.7338 13.475 15.95 17.6M13.475 8.525C13.475 9.89191 12.3669 11 11 11C9.6331 11 8.525 9.89191 8.525 8.525C8.525 7.1581 9.6331 6.05 11 6.05C12.3669 6.05 13.475 7.1581 13.475 8.525ZM19.25 11C19.25 15.5563 15.5563 19.25 11 19.25C6.44365 19.25 2.75 15.5563 2.75 11C2.75 6.44365 6.44365 2.75 11 2.75C15.5563 2.75 19.25 6.44365 19.25 11Z" stroke="#4F46E5" stroke-width="1.6" />
                        </svg>
                        <input id="message-input" wire:input.debounce.100ms="userTyping" wire:model="message" class="flex-1 text-black text-sm font-medium leading-4 focus:outline-none bg-transparent" placeholder="Type here...">
                        <div class="flex items-center gap-2">
                            ...
                            <button type="submit" class="flex items-center px-4 py-2 bg-indigo-600 rounded-full shadow hover:bg-indigo-700 transition">
                                ...
                                <span class="text-white text-xs font-semibold pl-2">Send</span>
                            </button>
                        </div>
                    </div>
                </form>
...
<script type="module">
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Listen for UserTyping event on the receiver's channel
        window.Echo.private(`chat-channel.{{ auth()->id() }}`).listen('UserTyping', (event) => {
        ...
        function scrollToLatestMessage() {
            ...
            if (chatInput) {
                ...
                // console.log("Scrolled page to chat input");
            } else {
                window.scrollTo({
                    top: document.body.scrollHeight,
                    behavior: 'smooth'
                });
                // console.log("Scrolled page to bottom (fallback)");
            }
        }

        scrollToLatestMessage(); // On page load

        Livewire.on('messages-updated', () => {
            // console.log("Messages updated event fired");
            setTimeout(() => {
                scrollToLatestMessage(); // After new message
            }, 500);
        });
